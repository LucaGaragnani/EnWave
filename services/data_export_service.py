...
def connect_to_db():
    ...
    db_directory = current_directory.replace('services', 'database')
...
def fetch_asset_data(cursor):
    ...
    cursor.execute("SELECT id,tag,function, manufactorer, yom, yoi, rated_voltage,feature_1,feature_2,feature_3,feature_4,feature_5,feature_6,feature_7,feature_8,feature_9,feature_10,feature_11,feature_12,feature_13, lat, lon FROM asset")  # Modify as per your table and columns
...
def fetch_offline_test_data(cursor):
    ...
    SELECT 
        otd.*, 
        asc.name AS subcomponent_name, 
        dt.name AS diagnostic_test_name
    FROM 
        offline_test_data otd
    JOIN 
        asset_subcomponent asc ON otd.subcomponent_id = asc.id
    JOIN 
